...
import data from "../../public/transcribe.json";
...
export const Main = ({
  ...
  const itemMaps = data.results.items
    .map((v) => {
      return {
        start_time: v.start_time,
        end_time: v.end_time,
        text: v.alternatives[0].content,
      };
    })
    .filter((v) => v.start_time != undefined || v.end_time != undefined);
  ...
  const captionToTransition = useMemo(() => {
    ...
    caption.map((cap, idx) => {
      ...
      cap.text.split(" ").map((word) => {
        const checkIndex = itemMaps.findIndex((v) => v && v.text == word);
        let frameMap = null;
        if (checkIndex > -1) {
          frameMap =
            Math.ceil((parseFloat(itemMaps[checkIndex].end_time as string) -
              parseFloat(itemMaps[checkIndex].start_time as string)) *
            fps);
          delete itemMaps[checkIndex];
        }
        if(word.includes(".")){
          tempFrame += 50
        }
